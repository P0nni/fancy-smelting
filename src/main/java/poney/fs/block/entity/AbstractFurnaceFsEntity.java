...
public abstract class AbstractFurnaceFsEntity extends BlockEntity implements GeoBlockEntity, ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    public int maxTimefuel (){
        if(AbstractFurnaceFsBlock.isEmptyFuel(getCachedState())) return 0;

        return AbstractFurnaceFsBlock.getCurrentFuel(getCachedState()) == FurnaceFuel.LAVA ? 100000 : 50000;
    }
    ...
    public AbstractFurnaceFsEntity(BlockPos pos, BlockState state,BlockEntityType<?> type) {
        ...
        this.propertyDelegate = new PropertyDelegate() {
            ...
            public int get(int index) {
                return switch (index) {
                    case 0 -> AbstractFurnaceFsEntity.this.progress;
                    case 1 -> AbstractFurnaceFsEntity.this.maxProgress;
                    case 2 -> AbstractFurnaceFsEntity.this.timeFuel;
                    case 3 -> AbstractFurnaceFsEntity.this.maxTimefuel();
                    default -> 0;
                };
            }
            ...
            public void set(int index, int value) {
                switch (index) {
                    case 0 -> AbstractFurnaceFsEntity.this.progress = value;
                    case 1 -> AbstractFurnaceFsEntity.this.maxProgress = value;
                    case 2 -> AbstractFurnaceFsEntity.this.timeFuel = value;
                }
            }
    ...
    public void tick(World world, BlockPos blockPos, BlockState blockState) {
        if(world.isClient()){
            ...
            if(!AbstractFurnaceFsBlock.isEmptyFuel(blockState)){
                this.increaseTimeFuel();
                if(timeFuel >= maxTimefuel()){
                    AbstractFurnaceFsBlock.changeProperties(world,blockPos,blockState,FurnaceFuel.EMPTY,false,0);
                    this.resetTimeFuel();
                }
    ...
    private static int getCookTime(World world, AbstractFurnaceFsEntity furnace) {
    ...
    private void increaseCraftProgress(){
        progress += getFurnaceLevel().asLevel();
    }
    ...
    private boolean hasRecipe() {
        ...
        if(hasRecipe && !AbstractFurnaceFsBlock.isLit(getCachedState())) {
            AbstractFurnaceFsBlock.changeProperties(world, getPos(), getCachedState(), AbstractFurnaceFsBlock.getCurrentFuel(getCachedState()), true, 13);
        }else if(!hasRecipe && AbstractFurnaceFsBlock.isLit(getCachedState())){
            AbstractFurnaceFsBlock.changeProperties(world, getPos(), getCachedState(), AbstractFurnaceFsBlock.getCurrentFuel(getCachedState()), false, 13);
        }
    ...
    public boolean isLit(){return AbstractFurnaceFsBlock.isLit(getCachedState());}
    public FurnaceFuel getCurrentFuel(){return AbstractFurnaceFsBlock.getCurrentFuel(getCachedState());}
